...
2. `<cwd>/.post-to-wechat/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.post-to-wechat/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .post-to-wechat/.env (this project only)
B) User-level: ~/.post-to-wechat/.env (all projects)
```
